PROGRAM MAIN
    ...
    ! Create a subgroup (just a group within a group)
    CALL h5gcreate_f(group_id, subgroup, subgroup_id,error)

    ! Create the dataspace
    CALL h5screate_simple_f(drank1, dims1, dspace1,error)

    ! Create the dataset within the subgroup
    CALL h5dcreate_f(subgroup_id, dname1, H5T_NATIVE_INTEGER, dspace1, &
       dset1, error)
